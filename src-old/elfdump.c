...
static void print_usage(void)
{
    ERROR("Usage: elfdump [-g] [-o OUTPUT] [-p] [-s SYNTAX] [-v] OBJFILE [VADDR]\n"
            "\n"
            "elfdump v" ELFDUMP_VERSION ": little-endian MIPS ELF object file disassembler\n"
            "\n"
            "Optional arguments:\n"
...
static range parse_elf(disasm_state *state, unsigned char *data, long file_len, arg_config *args)
{
    ...
    if (ehdr->e_ident[EI_DATA] != 1 || u16le(ehdr->e_machine) != 8)
    {
         ERROR("Not little-endian MIPS.\n");
         exit(EXIT_FAILURE);
    }

    if (u16le(ehdr->e_shstrndx) == 0)
    {
...
#define SECTION(index) (Elf32_Shdr *)(data + u32le(ehdr->e_shoff) + (index) * u16le(ehdr->e_shentsize))
#define STR(strtab, offset) (const char *)(data + u32le(strtab->sh_offset) + offset)

    str_shdr = SECTION(u16le(ehdr->e_shstrndx));
    for (int i = 0; i < u16le(ehdr->e_shnum); i++)
    {
        shdr = SECTION(i);
        const char *name = STR(str_shdr, u32le(shdr->sh_name));

        if (memcmp(name, ".text", 5) == 0)
        {
            text_offset = u32le(shdr->sh_offset);

            if (!args->has_vaddr)
                out_range.vaddr = u32le(shdr->sh_addr);

            vaddr_adj = out_range.vaddr - u32le(shdr->sh_addr);
            out_range.length = u32le(shdr->sh_size);
            out_range.start = text_offset;
            text_section_index = i;
        }

        if (u32le(shdr->sh_type) == SHT_SYMTAB)
    ...
    if (symtab_section_index != -1)
    {
        ...
        sym_strtab = SECTION(u32le(sym_shdr->sh_link));

        assert(u32le(sym_shdr->sh_entsize) == sizeof(Elf32_Sym));

        for (unsigned int i = 0; i < u32le(sym_shdr->sh_size); i += sizeof(Elf32_Sym))
        {
            Elf32_Sym *sym = (Elf32_Sym *)(data + u32le(sym_shdr->sh_offset) + i);
            const char *name = STR(sym_strtab, u32le(sym->st_name));
            uint32_t addr = u32le(sym->st_value);

            if (u16le(sym->st_shndx) != text_section_index || name[0] == '.')
                continue;
    ...
    for (int i = 0; i < u16le(ehdr->e_shnum); i++)
    {
        ...
        if (u32le(shdr->sh_type) != SHT_REL || u32le(shdr->sh_info) != (unsigned int) text_section_index)
            continue;

        assert(u32le(shdr->sh_link) == (unsigned int) symtab_section_index);
        assert(u32le(shdr->sh_entsize) == sizeof(Elf32_Rel));

        for (unsigned int i = 0; i < u32le(shdr->sh_size); i += sizeof(Elf32_Rel))
        {
            Elf32_Rel *rel = (Elf32_Rel *)(data + u32le(shdr->sh_offset) + i);
            uint32_t offset = text_offset + u32le(rel->r_offset);
            uint32_t symIndex = ELF32_R_SYM(u32le(rel->r_info));
            uint32_t rtype = ELF32_R_TYPE(u32le(rel->r_info));
            const char *symName = "0";

            if (symIndex != STN_UNDEF && sym_shdr != NULL && sym_strtab != NULL)
            {
                Elf32_Sym *sym = (Elf32_Sym *)(data + u32le(sym_shdr->sh_offset) + symIndex * sizeof(Elf32_Sym));
                symName = STR(sym_strtab, u32le(sym->st_name));
            }
            else
            {
                // TODO: set symName to something like "s_<address>"
                // printf("unknown symbol %08x\n", rel);
            }
            ...
            if (rtype == R_MIPS_LO16)
            {
                ...
                if (prevHi != NULL)
                {
                    uint32_t offset2 = text_offset + u32le(prevHi->r_offset);
            ...
            {
                int32_t addend = (u32le(*(uint32_t*)(data + offset)) & ((1 << 26) - 1)) << 2;
